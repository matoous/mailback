...
func ExactMonthDate(s rules.Strategy) rules.Rule {
	...
	return &rules.F{
		RegExp: regexp.MustCompile("(?i)" +
			"(?:\\W|^)" +
			"(?:(?:(" + OrdinalWordsPattern[3:] + "(?:\\s+of)?|([0-9]+))\\s*)?" +
			"(" + MonthOffsetPattern[3:] + // skip '(?:'
			"(?:\\s*(?:(" + OrdinalWordsPattern[3:] + "|([0-9]+)))?" +
			"(?:\\W|$)",
		),
